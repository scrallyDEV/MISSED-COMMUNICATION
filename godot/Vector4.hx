...
**/
package godot;
@:forward @:forward.new @:forwardStatics @:copyType extern abstract Vector4(godot.Vector4_Fields) {
	@:arrayAccess
	...
	@:op(A in B)
	public static inline function inOp2(self:godot.Vector4, other:GodotArray):Bool {
		return untyped #if gdscript __gdscript__ #else __cpp__ #end("(({0}) in ({1}))", self, other);
	}
	@:op(A in B)
	public static inline function inOp3(self:godot.Vector4, other:godot.PackedVector4Array):Bool {
		return untyped #if gdscript __gdscript__ #else __cpp__ #end("(({0}) in ({1}))", self, other);
	}
}