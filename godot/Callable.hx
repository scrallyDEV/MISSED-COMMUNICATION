...
**/
package godot;
@:forward @:forward.new @:forwardStatics @:copyType extern abstract Callable(godot.Callable_Fields) {
	@:op(A == B)
	...
	@:op(A in B)
	public static inline function inOp2(self:godot.Callable, other:GodotArray):Bool {
		return untyped #if gdscript __gdscript__ #else __cpp__ #end("(({0}) in ({1}))", self, other);
	}
}